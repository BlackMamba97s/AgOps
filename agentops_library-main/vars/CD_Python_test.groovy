def call(body) {
    ...
    pipeline {
        ...
        parameters {
            booleanParam(name: 'newRag', defaultValue: 'true', description: "Vuoi rieseguire il RAG?")
            booleanParam(name: 'newPrompt', defaultValue: 'true', description: "Vuoi caricare un nuovo prompt?")
            booleanParam(name: 'onlyTest', defaultValue: 'false', description: "Vuoi eseguire solo i test?")
            booleanParam(name: 'forceTag', defaultValue: 'false', description: "Vuoi forzare la generazione del tagi in caso di test? Il tag verrà generato solo se non esistente")
        }
        ...
        stages {
            stage('Preliminary steps') {
                steps {
                    script {
                        ...
                        DEPLOYMENT_FILE_NAME = infoMap.deployment_filename_test
                        TEST_ENDOPOINT = infoMap.test_endpoint
                        ...
                        newRag = params.newRag
                        namespace = infoMap.namespace_test
            ...
            stage('Checking components to update'){
                steps{
                        ...
                        script{
                            ...
                            if (!params.agentVersion.startsWith("R")){
                                withCredentials([usernamePassword(credentialsId: 'clustervigil_gitlab', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                                    ...
                                    if (tagExists == 0) {
                                        echo "Il tag '${versionTag}' esiste già nel repository. Quindi la combinazione è già stata usata in test"
                    ...
                    script {
                        ...
                        if (params.onlyTest == true){ //only test to execute
                        ...
                        else if (agentVersion_check.startsWith("R")){ //rollback
                            ...
                            operation = "rollback"
                            newRag = true //force to check if the rollback needs to update rag
                            newPrompt = true //force to check if the rollback needs to update prompt
                        }
                        ...
                        if (newRag == true && params.onlyTest == false){
                            ...
                            echo """newRag == false, RAG will not be ${operation} """
                            ragId = ragId_current
                            newRag = false
                        }
                    
                        if (newPrompt == true && params.onlyTest == false){
                            ...
                            echo """newPrompt == false, Prompt will not be ${operation}"""
                            promptVersion = promptVersion_current
                            newPrompt = false
                        }
                        if (params.forceTag == true && params.onlyTest == true){
                            withCredentials([usernamePassword(credentialsId: 'clustervigil_gitlab', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_PASS')]) {
                                def versionAgregated="R"+agentVersion+"_"+ragId+"_"+promptVersion
                                def tagExists = sh(
                                    script: """
                        ...
                        echo """
                            ...
                            GENERATE NEW TAG:   ${!agentVersion_check.startsWith("R") && params.onlyTest == false && deploy_update_required==true || forceTag_check}
                            TAG TO GENERATE:    ${"R"+agentVersion+"_"+ragId+"_"+promptVersion}
                            ------------------------------
                        """
                    }
                }
                ...
                steps {
                    script {
                        ansiColor('xterm') {
                            ...
                            //createRAG(ragId, ragDir, ragSize, namespace, pvcNameId, azure_storage_account)
                        }
                    }
                }
            }
            ...
            stage('Tag repository') {
                when {expression {!agentVersion_check.startsWith("R") && params.onlyTest == false && deploy_update_required==true || forceTag_check}}
                agent {
                    label "${SLAVE_LABEL}"
                }
                steps {
